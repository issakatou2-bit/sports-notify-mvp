...
# その線まで、勝ち点で何点離れるまでを話として扱うか。
#
# **順位の差で切っていたのを、9/13に勝ち点の差へ変えた。**
#
# 本番で初めて分かった。ラ・リーガが6試合まで進んだ日、
# 久保建英のレアル・ソシエダは11位。線（CL圏内は4位、EL圏内は5位）
# からは6〜7つ離れているので「遠すぎる」として落としていた。
#
# ところが勝ち点で見ると、
#
#     4位 セビージャ        勝点10
#     5位 レアル・ベティス   勝点 9
#    11位 レアル・ソシエダ   勝点 7   ← 久保建英
#
# **EL圏内まで勝点2、CL圏内まで勝点3。1勝で届く距離だった。**
# 9月は全体が詰まっていて、順位が6つ違っても勝ち点は2しか
# 違わない。順位の差で切ると、いちばん話になる時期に何も
# 出せなくなる。
#
# 6点は2勝ぶん。シーズンが進めば順位差と勝ち点差は一緒に
# 広がるので、終盤は自然に落ちる。
MAX_POINT_GAP = 6
    ...
    for row in rows:
        ...
        for p in players:
            ...
            for at, label in cutline.lines_for(code):
                ...
                if not d:
                    continue
                # 線から遠すぎるものは話にしない。**勝ち点で見る。**
                # 順位の差で見ると、全体が詰まっている時期に
                # 1勝で届く相手まで落としてしまう。
                if (d.get("diff") or 0) > MAX_POINT_GAP:
                    continue
