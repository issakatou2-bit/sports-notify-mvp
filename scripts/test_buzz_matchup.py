...
# 訳した題の中の、日本人選手の名前。
#
# 「選手名はカタカナにする」と頼んでいるので、日本人選手まで
# 「ムネタカ・ムラカミ」で返ってきて、それが動画の題になっていた。
# 題に名前があるかどうかで再生は2.8倍、登録者は12対1違うが、
# その名前は漢字でなければ検索にも記憶にも届かない。
# ------------------------------------------------------------------
print(chr(10) + "=== 訳文の日本人選手名 ===")
import mentioned  # noqa: E402


def _k(label, got, want):
    global fails
    ok = got == want
    fails += not ok
    print(f"{'ok ' if ok else 'NG '} {label}: {got!r}"
...
_k("名・姓の順", mentioned.to_kanji("ムネタカ・ムラカミが歴史的快挙"),
   "村上宗隆が歴史的快挙")
_k("姓・名の順", mentioned.to_kanji("ムラカミ・ムネタカが30号"), "村上宗隆が30号")
_k("姓だけ", mentioned.to_kanji("ムラカミが2打席連続本塁打"),
   "村上宗隆が2打席連続本塁打")
_k("外国人選手はそのまま", mentioned.to_kanji("カル・ローリーが4打席連続本塁打"),
   "カル・ローリーが4打席連続本塁打")
_k("同じ文に2人", mentioned.to_kanji("オオタニとヤマモトが並んだ"),
   "大谷翔平と山本由伸が並んだ")
_k("空文字でも落ちない", mentioned.to_kanji(""), "")
