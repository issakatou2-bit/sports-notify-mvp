...
DAILY_LINEUP = [
    ...
    # 「今日の1人」を9/2に畳んで、進出争いに入れ替えた。
    #
    # 28日で156回/本・継続23.5%・登録0人。下から2番目で、
    # 「この枠は何をする枠なのか」に答えが出ないままだった。
    # 出しているのは通算・今季・昨季・直近・受賞歴で、
    # Wikipediaの要約に近い。数字は正しいが「へえ」が起きない。
    #
    # 進出争いは9月と10月だけの主題で、毎日1つずつ数字が減る。
    # 期間限定なので、外して駄目なら戻せる。
    ("postseason", "ポストシーズン進出争い","マジックと今日終わったらの組み合わせ", "20:00"),
    # **2026-09-15に30分刻みへ。**18:00に2本、19:00に2本が重なっていて、
    # 同じ時刻に出した片方はフィードでも並びでも埋もれる。
    # 17:00から21:00まで30分ずつずらせば、本数を減らさずに全部が
    # 単独の時刻を持てる。22:00と半端な時刻はやめる、というのが
    # ユーザーの意向（「21時までの時間帯でやりたい」）。
    ("morning_voices", "ファンのコメント欄", "最も見られたハイライトの反応を翻訳", "17:30"),
    ("daily", "明日の注目試合", "なぜ注目なのかの理由つき", "19:00"),
    # サッカーは「今夜のキックオフ前」に出す枠なので、19:30でも遅くない。
    ("daily_soccer", "欧州サッカー", "その夜の注目カード", "19:30"),
    ("morning_press", "現地の報道", "番記者の投稿と見出しを翻訳", "18:00"),
    # 長編（3分）。ショートではないが、毎日出る枠なので一覧に入れる。
    #
    # ここは「健康診断・再生リスト・その日ページ・読み上げ」が
    # 全部見ている正本。片方だけに足すと検査が落ちる（実際落ちた）。
    # 9/14に主題を公式コメント欄から「その日の数字」へ変えた。
    # 名前がここに残っていると、健康診断もその日ページも古い名前を出す。
    # 横型で3分。腰を据えて見るものなので、いちばん後ろに置く。
    ("longform", "きょうのMLB、数字で（3分）",
     "ずんだもんと四国めたんが成績と進出争いを追う", "21:00"),
]
