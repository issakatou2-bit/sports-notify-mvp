...
def _pick_from_dialogue(path: str) -> str:
    """サムネイルに載せる一言。いちばん支持されたコメントの訳。

    「何の動画か」より「何が言われているか」のほうが、
    一目で押す理由になる。読ませるためではなく、
    **中で何が読まれるかの見本**として置く。
    """
    if not path:
        return ""
...
def draw_longform(im, d, topic: str, day: str, portrait_dir: str,
    ...
    # いちばん支持されたコメントを1つ。
    #
    # 説明の1行（「公式コメント欄を読む」）より、こちらのほうが
    # 押す理由になる。**中で何が読まれるかの見本**として置く。
    # 引用なので、カギ括弧に入れて「翻訳」と添える。
    if pick:
        pw = 880 - 120 - 70
        ps, body = 40, "「" + pick + "」"
        li = video_common.wrap(d, body, font(ps), pw)
        while len(li) > 2 and ps > 26:
            ps -= 6
            li = video_common.wrap(d, body, font(ps), pw)
        for ln in li[:2]:
