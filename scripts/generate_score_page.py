...
def render() -> str:
    ...
    clutch_rows = "".join(
        ...
        # 点数順ではなく、呼び方の優先順に並べる。
        # 本文で「上にあるものを使う」と書いているので、
        # 表の並びがその順でないと説明と食い違う。
        for k, v in sorted(clutch.CLUTCH_POINTS.items(),
                           key=lambda x: (clutch.CLUTCH_ORDER.index(x[0])
                                          if x[0] in clutch.CLUTCH_ORDER
                                          else 99))
    )
...
スコア = 基準点 + 素点 × 1.2</div>
  ...
  <h2>打者</h2>
  <div class="formula">塁打 = 単打 + 2 × 二塁打 + 3 × 三塁打 + 4 × 本塁打
       ↑ MLB公式の集計値(totalBases)をそのまま使っています

凡退 = 打数 - 安打

素点 = 2 × 塁打 + {mr.HR_BONUS} × 本塁打 + 2 × 打点 + 2 × 四球 + 2 × 死球
       + 盗塁 - 凡退 - 三振
スコア = {mr.BATTER_BASE} + 素点 × {mr.BATTER_SCALE}</div>
  ...
  <p class="note">本塁打には塁打(4)とは別に{mr.HR_BONUS}点を足しています。
  塁打だけで見ると本塁打は単打4本ぶんですが、4本打つことと1本で1点を
  入れることは記録としての意味が違います。この加点を入れる前は、
  5打数1安打がその1本が本塁打だった日でも68点で、単打だけの日と
  大差が付きませんでした。いまは86点です。</p>
  ...
  1日に複数該当した場合は、それぞれ加算されます。
  ただし1つの打席に複数当てはまるときは、上の表で上にある呼び方を
