...
# 1試合の成績を語っている文の、1試合ぶんの単位。
#
# 9/11に**正しい台詞を止めた。**「山本由伸が7回10奪三振」に対して
# 「材料は171奪三振です」と言って長編を落とした。台詞はその日の
# 登板の話で、材料が持っているのは今季の累計。**別のものを
# 突き合わせていた。**
#
# 投球回が併記されている文は、1試合の成績を語っている。
# 「7回10奪三振」「6.2回3被安打」のような形。そこに出る
# 奪三振・被安打・自責・四球は今季の累計とは比べない。
#
# 今季の累計を語る文（「今季171奪三振」）には投球回が付かないので、
# そちらはこれまでどおり照合される。
#
# 止める側が間違っていると、正しい日に何も出ない。
# 検算(sanity)はそれを4回やっていて、ここで5回目になった。
GAME_UNITS = ("奪三振", "被安打", "安打", "自責", "四球", "打点")
GAME_INNINGS = re.compile(r"[0-9]+(?:[.][0-9]+)?回")
...
def check(d: dict) -> list:
    ...
    for seg in (d.get("segments") or []):
        ...
        # 投球回が併記されている文は、1試合の成績を語っている。
        one_game = bool(GAME_INNINGS.search(text))
        for name, stats in facts.items():
            ...
            for unit in UNITS:
                ...
                if want is None:
                    continue
                if one_game and unit in GAME_UNITS:
                    continue         # その登板の数字。今季の累計とは別
