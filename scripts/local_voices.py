...
def translate(client, items: list) -> list:
    ...
    prompt = (
        ...
        "中立=事実の指摘や質問\n"
        "- **日本語として自然に読めることを先に取る。逐語訳にしない。**\n"
        "  英語の語順や言い回しをなぞると、読んでも意味が入ってこない\n"
        "- **書き手の熱量は、日本語の言い回しで作り直す。**\n"
        "  興奮・怒り・呆れ・喜びが、訳文を読んだだけで伝わるようにする\n"
        "    LFG!!! → 「行くぞーーー!!!」（「さあ行こう」ではない）\n"
        "    bro is cooked → 「あいつもう終わってる」\n"
        "    no way he did that → 「うそだろ、いまの」\n"
        "- **絵文字はそのまま残す。**熱量の一部なので落とさない\n"
        "- 選手名・球団名は英語のままでよい。ただし\n"
