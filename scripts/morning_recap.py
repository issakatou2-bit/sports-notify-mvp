...
def headline_bits(row: dict):
    ...
    if row["type"] == "pitcher":
        ...
        # 被安打と与四球も出す。
        #
        # 「1.0回　2奪三振　自責0」だけを見せていた。点数はこの裏で
        # 被安打2点、与四球1点を引いている。画面はきれいな1回に見えるのに
        # 順位が下なので、数字が勝手に動いているように見える。
        # 引かれている理由は、引かれた本人と同じ画面に出す。
        # 被安打は出す。与四球は落とす。
        #
        # 点を引いている理由を見せるのが目的だが、全部並べると
        # 33字になって画面からはみ出す。走者を背負った事実は
        # 被安打だけでも伝わり、与四球は防御率にも効いている。
        # 1行に入る量には上限があるので、そこは選ぶ。
        if row.get("hits"):
            bits.append(f"{row['hits']}被安打")
        # 失点した登板は、自責の代わりにその試合の防御率を出す。
        #
        # 両方並べると「5.0回 4奪三振 自責5 7被安打 1与四球 防御率9.00
        # 負け投手」で36字になり、いちばん小さい文字でも画面から
        # はみ出した。防御率は自責と投球回を両方含んでいるので、
        # 出すなら片方でよい。
        outs_ = outs_from_ip(row.get("ip"))
        if row.get("er") and outs_:
            era = row["er"] * 27 / outs_
            bits = [b for b in bits if not b.startswith("自責")]
            bits.insert(2, f"防御率{era:.2f}")
        # 役割は点数を左右するので、画面にも出す。
        ...
        # 勝ち投手だけ出す。負け投手は出さない。
        #
        # 成績の行に使える幅は520px。そこへ入るのは24字ほどで、
        # 「3.0回 2奪三振 防御率15.00 9被安打 負け投手」は28字あって
        # 最小の文字でもはみ出す。落とす順に理屈が要る。
        #
        # 負けは既に防御率と被安打に出ていて、点数にも効いている。
        ...
        # それでも入らない日は、被安打を落とす。
        #
        # 9/10の山本由伸「7.0回　10奪三振　防御率1.29　3被安打　勝ち投手」が
        # 28字で、**いちばん小さい文字でも520pxに入らなかった。**
        # 上のコメントで「被安打は出す」と決めていたが、落とす余地が
        # ホールドとセーブ失敗しか無かったので、好投した日ほど
        # 行が長くなって、はみ出すことになっていた。
        #
        # 被安打は防御率の中に既に入っている（自責は被安打から生まれる）。
        # 勝ちは他のどの数字にも出ないので、最後まで残す。
        drops += [i for i, b in enumerate(bits) if b.endswith("被安打")]
        return bits, drops
...
# 読み上げに渡す呼び名。**全部の記録に書く。**
#
# QSとHQSだけ書いてあったが、長編は「HQS」をそのまま渡されて
# 「ヒットクオリティスタート」と読んだ（正しくはハイクオリティスタート）。
# ユーザーの提案:「ネームド記録に読み仮名もつければいける？」——そのとおりで、
# **辞書にあれば、読み方を作る余地がなくなる。**
#
# 変わらないものも書いておく。「ここに無い語は渡していない」と
# 言えるようにするため（検査もこの辞書を見る）。
BADGE_SPEECH = {
    "QS": "クオリティスタート",
    "HQS": "ハイクオリティスタート",
    "ノーヒッター": "ノーヒットノーラン",
...
def contribution(row: dict) -> int:
    ...
    if row.get("type") == "pitcher":
        ...
        # 四球は単打より軽くする。
        #
        # 2点は塁打1つと同じで、つまり単打とまったく同じ重さだった。
        # そのため「3打数0安打　2四球」が「5打数1安打　1二塁打」の
        # 上に来ていた。四球で塁に出るのは確かな貢献だが、走者を
        # 進めも返しもしない点で単打とは違う。
        # 得点期待値の標準的な重み(四球0.55、単打0.70)に合わせて8割にする。
        raw = (2 * tb
               + HR_BONUS * row.get("hr", 0)
               + 2 * row.get("rbi", 0)
               + WALK_VALUE * row.get("bb", 0)
               + WALK_VALUE * row.get("hbp", 0)
               + 1 * row.get("sb", 0)
               - outs_made
               - row.get("so", 0))
        ...
        # 3本塁打で260点前後、無安打なら50を割って非表示側へ落ちる。
...
# 完封級・2本塁打級がここに入る。
#
# 9/11に倍率をそろえたあとの分布（26日・151人日）:
#   投手 中央55 上位10%=100 100超え12% 130超え 2%
#   打者 中央48 上位10%=102 100超え12% 130超え 4%
# 130を超えるのは月に数回で、「突き抜けた日」の名に合っている。
STANDOUT = 130
...
# 四球の重み。単打(塁打1×2=2点)の8割。
# 得点期待値では四球0.55、単打0.70で、比はおよそ0.79。
WALK_VALUE = 1.6

# 投手の倍率。
#
# 1.2 だった。26日・151人日で分布を数えたところ、
#
#     投手  中央50  上位10%=86   最大115  100超え 2%
#     打者  中央47  上位10%=108  最大197  100超え16%
#
# **投手は100点にほとんど届かなかった。**完封が162点のとき
# 打者の2本塁打が182点で、その上に来ていた。名前のある記録の
# 加点で先発の好投は救えたが、倍率そのものの差は残っていた。
#
# 9/11にユーザーの承認を得て変えた。「スコアは記録として残す
# ものでもないので倍率は変えて大丈夫。並びに意味をもたせるもの
# だし、あまり押し出しすぎても新規視聴者には伝わりづらい」。
# それで**数字を大きくするのではなく、両方を寄せて**そろえた。
PITCHER_SCALE = 1.8

BATTER_BASE = 50

# 本塁打そのものへの加点。塁打(8点)とは別に足す。
#
# 塁打だけで見ると、本塁打は単打4本ぶんでしかない。だが4本打つのと
# 1本で1点を入れるのは、記録としての意味が違う。実際、5打数1安打が
# 本塁打だった日が68点で、単打だけの日と大差が付かなかった。
#
# 5を足すと、その日は81点になる。2本打てば160点台。
# 1本出た日は「その日の目玉」として並ぶ。
HR_BONUS = 5
BATTER_SCALE = 3.1
