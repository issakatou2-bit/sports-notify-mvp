...
def build_output(
    ...
    for g in games:
        ...
        def _standing_dict(team_id):
            ...
            if not s:
                return None
            # サッカーで開幕直後は、順位そのものが意味を持たない。
            #
            # 全チームが勝ち点0で並び、APIは得失点差などのタイブレークで
            # 順位を付けて返すだけ。AIへ渡す側では既に切っているが
            # (_soccer_context_line)、**JSONに残っていると画面や
            # 読み上げがそれを拾う。**9/11のCL第1節では、
            # マンチェスター・ユナイテッドとSabah FKの両方に
            # 「13位・1.0ゲーム差・勝ち点差3.0」が入っていた。
            # 同じ境目でここでも切る。
            if is_soccer(g) and (s.played or 0) < SOCCER_TABLE_MIN_MATCHES:
                return None
            return {
...
JP_PLAYERS_MLB = [
    {"name_en": "Shohei Ohtani", "name_jp": "大谷翔平", "kana": "オオタニ・ショウヘイ", "type": "batter"},
    {"name_en": "Yu Darvish", "name_jp": "ダルビッシュ有", "kana": "ダルビッシュ・ユウ", "type": "pitcher"},
    {"name_en": "Roki Sasaki", "name_jp": "佐々木朗希", "kana": "ササキ・ロウキ", "type": "pitcher"},
    {"name_en": "Yoshinobu Yamamoto", "name_jp": "山本由伸", "kana": "ヤマモト・ヨシノブ", "type": "pitcher"},
    {"name_en": "Tomoyuki Sugano", "name_jp": "菅野智之", "kana": "スガノ・トモユキ", "type": "pitcher"},
    {"name_en": "Yusei Kikuchi", "name_jp": "菊池雄星", "kana": "キクチ・ユウセイ", "type": "pitcher"},
    {"name_en": "Shota Imanaga", "name_jp": "今永昇太", "kana": "イマナガ・ショウタ", "type": "pitcher"},
    {"name_en": "Seiya Suzuki", "name_jp": "鈴木誠也", "kana": "スズキ・セイヤ", "type": "batter"},
    {"name_en": "Kodai Senga", "name_jp": "千賀滉大", "kana": "センガ・コウダイ", "type": "pitcher"},
    {"name_en": "Yuki Matsui", "name_jp": "松井裕樹", "kana": "マツイ・ユウキ", "type": "pitcher"},
    # 2026/7時点で判明した見落とし分。2026年在籍16名のうち欠けていた6名を追加
    {"name_en": "Masataka Yoshida", "name_jp": "吉田正尚", "kana": "ヨシダ・マサタカ", "type": "batter"},
    {"name_en": "Kazuma Okamoto", "name_jp": "岡本和真", "kana": "オカモト・カズマ", "type": "batter"},
    {"name_en": "Munetaka Murakami", "name_jp": "村上宗隆", "kana": "ムラカミ・ムネタカ", "type": "batter"},
    ...
    #
    # 9/9は日本語表記が確かめられずカタカナで入れた。**カタカナの
    # 人名は画面でも読み上げでも外国人選手に見える。**9/11に
    # ユーザーから漢字表記の指摘を受けて直した。
    # 読みは「にしだ りくう」。
    {"name_en": "Rikuu Nishida", "name_jp": "西田陸浮",
...
JP_PLAYER_READINGS = {
    ...
    "今井達也": "イマイ、タツヤ",
    "ヌートバー": "ヌートバー",
    # 「陸浮」は確実に誤読する。名簿をカナから漢字に直したので、
    # ここも同時に入れる（片方だけ直すと読み上げが崩れる）。
    "西田陸浮": "ニシダ、リクウ",
}
...
JP_PLAYERS_SOCCER = [
    ...
    # --- プレミアリーグ(イングランド) ---
    {"name_en": "Wataru Endo", "name_jp": "遠藤航", "kana": "エンドウ・ワタル", "team_en": "Liverpool",
     "team_jp": "リバプール", "league": "PL", "match": "liverpool"},
    {"name_en": "Kaoru Mitoma", "name_jp": "三笘薫", "kana": "ミトマ・カオル", "team_en": "Brighton",
     "team_jp": "ブライトン", "league": "PL", "match": "brighton"},
    {"name_en": "Daichi Kamada", "name_jp": "鎌田大地", "kana": "カマダ・ダイチ", "team_en": "Crystal Palace",
     "team_jp": "クリスタル・パレス", "league": "PL", "match": "crystalpalace"},
    {"name_en": "Kota Takai", "name_jp": "高井幸大", "kana": "タカイ・コウタ", "team_en": "Tottenham",
    ...
    {"name_en": "Ryunosuke Sato", "name_jp": "佐藤龍之介", "kana": "サトウ・リュウノスケ", "team_en": "Valencia",
     "team_jp": "バレンシア", "league": "PD", "match": "valencia"},

    # --- ブンデスリーガ(ドイツ) ---
    # "bayern" 単体だと Bayer Leverkusen とは別物なので衝突しない
    # (bayer と bayern で綴りが違う)。
    {"name_en": "Hiroki Ito", "name_jp": "伊藤洋輝", "kana": "イトウ・ヒロキ", "team_en": "Bayern Munich",
     "team_jp": "バイエルン", "league": "BL1", "match": "bayern"},
    {"name_en": "Koki Machida", "name_jp": "町田浩樹", "kana": "マチダ・コウキ", "team_en": "Hoffenheim",
     "team_jp": "ホッフェンハイム", "league": "BL1", "match": "hoffenheim"},
    {"name_en": "Yuito Suzuki", "name_jp": "鈴木唯人", "kana": "スズキ・ユイト", "team_en": "Freiburg",
     "team_jp": "フライブルク", "league": "BL1", "match": "freiburg"},
    {"name_en": "Ritsu Doan", "name_jp": "堂安律", "kana": "ドウアン・リツ", "team_en": "Eintracht Frankfurt",
     "team_jp": "フランクフルト", "league": "BL1", "match": "eintrachtfrankfurt"},
    {"name_en": "Kaishu Sano", "name_jp": "佐野海舟", "kana": "サノ・カイシュウ", "team_en": "Mainz",
     "team_jp": "マインツ", "league": "BL1", "match": "mainz"},
    {"name_en": "Sota Kawasaki", "name_jp": "川﨑颯太", "kana": "カワサキ・ソウタ", "team_en": "Mainz",
     "team_jp": "マインツ", "league": "BL1", "match": "mainz"},
    # "borussia" はドルトムントとぶつかるので地名側を使う
    {"name_en": "Shuto Machino", "name_jp": "町野修斗", "kana": "マチノ・シュウト",
     "team_en": "Borussia Monchengladbach", "team_jp": "ボルシアMG",
     "league": "BL1", "match": "monchengladbach"},
    {"name_en": "Zento Uno", "name_jp": "宇野禅斗", "kana": "ウノ・ゼント",
     "team_en": "Borussia Monchengladbach", "team_jp": "ボルシアMG",
     "league": "BL1", "match": "monchengladbach"},
    {"name_en": "Daiki Hashioka", "name_jp": "橋岡大樹", "kana": "ハシオカ・ダイキ",
     "team_en": "Borussia Monchengladbach", "team_jp": "ボルシアMG",
     "league": "BL1", "match": "monchengladbach"},
    {"name_en": "Satoshi Tanaka", "name_jp": "田中聡", "kana": "タナカ・サトシ", "team_en": "Schalke",
     "team_jp": "シャルケ", "league": "BL2", "match": "schalke"},

    # --- セリエA(イタリア) ---
    {"name_en": "Zion Suzuki", "name_jp": "鈴木ザイオン", "kana": "スズキ・ザイオン", "team_en": "Parma",
     "team_jp": "パルマ", "league": "SA", "match": "parma"},

    # --- リーグ・アン(フランス) ---
    {"name_en": "Takumi Minamino", "name_jp": "南野拓実", "kana": "ミナミノ・タクミ", "team_en": "Monaco",
     "team_jp": "モナコ", "league": "FL1", "match": "monaco"},
    {"name_en": "Ayumu Seko", "name_jp": "瀬古歩夢", "kana": "セコ・アユム", "team_en": "Le Havre",
     "team_jp": "ル・アーブル", "league": "FL1", "match": "havre"},
    {"name_en": "Sota Nakamura", "name_jp": "中村草太", "kana": "ナカムラ・ソウタ", "team_en": "Le Havre",
     "team_jp": "ル・アーブル", "league": "FL1", "match": "havre"},
...
SOCCER_CLUB_NAME_JP = {
    ...
    "qarabag": "カラバフ",
    "kairat": "カイラト",
    # 2026-09-09に追加。トップページのサッカータブに「Viking FK」と
    # 英語のまま出ていた。CL本戦に出てくるクラブは、毎年少しずつ
    # 入れ替わる。読み上げも英字を読めないので、出るたびに足す。
    "viking": "バイキング",
    "paphos": "パフォス",
...
def _build_ai_prompt(game: dict, standings: dict) -> str:
    ...
    return (
        ...
        # 9/11の実例。CLのリーグフェーズ第1節に対して
        # 「昨季3位のマンチェスター・ユナイテッド、開幕戦での出来が試金石」
        # というフック文が出た。CLの開幕としては正しいが、プレミアリーグは
        # 8月中旬に開幕して既に数節が終わっている。**どの大会の開幕なのかを
        # 書かないと、リーグ戦の開幕と読まれる。**
        "- 「開幕」「開幕戦」「初戦」と書くときは、どの大会の開幕なのかを"
        "  必ず添えること。サッカーはリーグ戦とカップ戦が並行して進み、"
        "  開幕日が別々にある。リーグ戦が8月に始まっていても"
        "  チャンピオンズリーグは9月に始まるので、「開幕戦」とだけ書くと"
        "  リーグ戦の開幕と受け取られる\n"
        "- 昨季の順位に触れるときも、どの大会の順位なのかを添えること。"
        "  「昨季3位」がリーグ戦の順位なのか別の大会のものなのかは、"
        "  書かなければ読む側に分からない\n"
